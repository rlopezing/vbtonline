...
public class AllMyPortafolioAction extends ActionSupport implements ServletContextAware, SessionAware,Serializable {
    ...
    public String cargar() throws Exception {
        ...
        try{
            ...
            allMyPortafolio=getAllMyPortafolioServicio().cargar("", data.getAllMyPortafolios().get(0));
...
//            data = gson.fromJson(this.jsonAllMyPortafolio, DataJson.class);
            ...
            carteras=getAllMyPortafolioServicio().cargarCarterasPortafolio(usuario.getLogin());
//            setMensaje(allMyPortafolio.getId().toString());
            ...
            portafolio = new ArrayList<>();
              for(int c=0;c<carteras.size();c++){
                  tablas = new ArrayList<>();
                  tabla = new TableOd();
                  tabla = getAllMyPortafolioServicio().cargarTablas(carteras.get(c),"verCuentas", usuario.getLogin(), idioma);
                  ...
                  tabla = new TableOd();
                  tabla = getAllMyPortafolioServicio().cargarTablas(carteras.get(c),"verColocaciones", usuario.getLogin(), idioma);
                  if(tabla.getContenidoTabla_info().size()>0){
                      tablaNombre = new TablasOd();
                      tablaNombre.setDatos(tabla);
                  ...
                  tabla = new TableOd();
                  tabla = getAllMyPortafolioServicio().cargarTablas(carteras.get(c),"verFondos", usuario.getLogin(), idioma);
                  if(tabla.getContenidoTabla_info().size()>0){
                      tablaNombre = new TablasOd();
                      tablaNombre.setDatos(tabla);
                  ...
                  tabla = new TableOd();
                  tabla = getAllMyPortafolioServicio().cargarTablas(carteras.get(c),"verOtrasInv", usuario.getLogin(), idioma);
                  if(tabla.getContenidoTabla_info().size()>0){
                      tablaNombre = new TablasOd();
                      tablaNombre.setDatos(tabla);
                  ...
                  tabla = new TableOd();
                  tabla = getAllMyPortafolioServicio().cargarTablas(carteras.get(c),"verTarjetas",  usuario.getLogin(),idioma);
                  if(tabla.getContenidoTabla_info().size()>0){
                      tablaNombre = new TablasOd();
                      tablaNombre.setDatos(tabla);
