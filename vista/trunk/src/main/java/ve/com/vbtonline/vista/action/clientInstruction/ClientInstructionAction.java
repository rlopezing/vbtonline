...
//            data = gson.fromJson(this.jsonAccounts, DataJson.class);
            ...
            setClients(getClientInstructionServicio().cargarClientes(user.getNumContrato(), idioma, user));

            setMaxCartas(getClientInstructionServicio().cargarListaMaxCartas(idioma, user));

            setVencimientos(getClientInstructionServicio().cargarListaVencimientos(idioma, user));

            setTransferTypes(getClientInstructionServicio().cargarListaTipoTransf(idioma, user));
...
//
    ...
    public String cargaValores() throws Exception {
        final String origen = "ClientInstructionAction.cargaValores";
        long time;
        AccountsOd account;
        String carteras = new String();
    ...
    public String evaluarCartaDeInstruccion() throws Exception {
        final String origen = "ClientInstructionAction.evaluarCartaDeInstruccion";
        long time;
        time = System.currentTimeMillis ();
        Gson gson=new Gson();
        ...
        try{
            ...
            VBTUsersOd user = (VBTUsersOd) session.get("user");
            idioma = (String) session.get("idioma");
            carta.setCliente(data.getParametros().get(4));
            carta.setRif(data.getParametros().get(1));
            carta.setTipoTransf(null);
            carta.setMaxcartas(null);
            carta.setVencimiento(null);
            carta.setCodperclicarta(data.getParametros().get(0));
            carta.setCodpercli(data.getParametros().get(0));
            carta.setDescTipoTransf(null);

            respuesta = clientInstructionServicio.evaluarNumeroControl( carta, null,  carta.getCodpercli(), user.getIP(),data.getParametros().get(3), user);
    ...
    public String anularCartaDeInstruccion() throws Exception {
        final String origen = "ClientInstructionAction.anularCartaDeInstruccion";
        long time;
        time = System.currentTimeMillis ();
        Gson gson=new Gson();
        data = gson.fromJson(this.jsonParametrosString, DataJson.class);
        ...
        try{
            ...
            respuesta = clientInstructionServicio.anularNumeroControl( carta, null,  carta.getCodpercli(), user.getIP(),data.getParametros().get(3),user.getLogin(), user);
