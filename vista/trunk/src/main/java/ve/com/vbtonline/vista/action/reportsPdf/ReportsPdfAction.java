...
public class ReportsPdfAction extends ActionSupport implements ServletContextAware, SessionAware,Serializable {
    ...
    public String reportEdoCuenta() throws Exception {
        ...
        try{
            ...
            float[] medidaCeldas = {7.8f,3.2f};
            PdfPTable TableCuentaCabecera = reportUtil.crearTabla(2,96,medidaCeldas);  // mueestra el Statement of Account y su respectiva imagen
            TableCuentaCabecera.setHorizontalAlignment(Element.ALIGN_LEFT);


            Paragraph tituloGrande = reportUtil.crearTituloGrande(18,"Savings Account");
            ...
            celltituloGrande.setPaddingTop(60f);
            celltituloGrande.setPaddingBottom(6f);
            celltituloGrande.setPaddingLeft(32f);
            ...
            archivo_temp += "/resources/images/comun/bgAC.jpg";

            PdfContentByte canvas = writer.getDirectContentUnder ( ) ;
            Image image = Image.getInstance (archivo_temp) ;
            image.scalePercent(72.0f, 70.0f);
            image.setAbsolutePosition(0, PageSize.A4.getHeight() - image.getScaledHeight()-45);
            ...
            TableCuentaCabecera.addCell(cell);




            //crear tabla declaracion de cuenta
            medidaCeldas = new float[]{1.86f, 1.2f};
            ...
            if(validar.validarCuenta2(cuentas,num_cta.split(" ", 2)[0].trim()).equalsIgnoreCase("SI")){
                cabecera = reportsPdfServicio.consultarCabeceraEdoCuenta(num_cta.split(" ", 2)[0].trim(),mes,anio, usuario);

                ///

                Paragraph tituloGrande2 = reportUtil.crearTituloGrande(14,"Statement of Account");
                PdfPCell celltituloGrande2 = new PdfPCell(tituloGrande2);
                celltituloGrande2.setHorizontalAlignment(Element.ALIGN_LEFT);
                celltituloGrande2.setBorder(Rectangle.NO_BORDER);
                celltituloGrande2.setColspan(2);
                celltituloGrande2.setPaddingLeft(16f);
                TableDeclaracion.addCell(celltituloGrande2);
                ...
                //cellPrimary.setPaddingLeft(32f);


                medidaCeldas = new float[]{1f};
                PdfPTable TableCuentaCabecera2 = reportUtil.crearTabla(1,96,medidaCeldas);  // mueestra el Statement of Account y su respectiva imagen
                TableCuentaCabecera2.setHorizontalAlignment(Element.ALIGN_LEFT);



                PdfPCell cellPrimary;
                cellPrimary = reportUtil.datosTabla("Primary(ies):", Element.ALIGN_LEFT);
                cellPrimary.setBorder(Rectangle.NO_BORDER);
                cellPrimary.setVerticalAlignment(Rectangle.ALIGN_TOP);
                TableCuentaCabecera2.addCell(cellPrimary);


                cellPrimary = reportUtil.datosTabla(cabecera.get(0), Element.ALIGN_LEFT);
                cellPrimary.setBorder(Rectangle.NO_BORDER);
                cellPrimary.setVerticalAlignment(Rectangle.ALIGN_TOP);
                TableCuentaCabecera2.addCell(cellPrimary);
                ...
                String clienteSecundarios="";

                String[] clientesSecundarios2;



                if (null!=cabecera.get(5)){

                    if ((cabecera.get(5).toString().trim().length()>0)){

                        clientesSecundarios2=cabecera.get(5).toString().trim().split("-");

                        for (int k=0;k<clientesSecundarios2.length;k++) {

                            cellPrimary = reportUtil.datosTabla(clientesSecundarios2[k].trim(), Element.ALIGN_LEFT);
                            cellPrimary.setBorder(Rectangle.NO_BORDER);
                            cellPrimary.setVerticalAlignment(Rectangle.ALIGN_TOP);
...
//
//                cellPrimary = reportUtil.datosTabla("HOLDMAIL", Element.ALIGN_LEFT);
//                cellPrimary.setBorder(Rectangle.NO_BORDER);
//                cellPrimary.setVerticalAlignment(Rectangle.ALIGN_TOP);
//                cellPrimary.setPaddingTop(6f);
//                TableCuentaCabecera2.addCell(cellPrimary);


                cellPrimary = reportUtil.datosTabla(cabecera.get(14), Element.ALIGN_LEFT);
                cellPrimary.setBorder(Rectangle.NO_BORDER);
                cellPrimary.setVerticalAlignment(Rectangle.ALIGN_TOP);
                cellPrimary.setPaddingTop(6f);
                TableCuentaCabecera2.addCell(cellPrimary);
               ...
               PdfPCell cellDeclaracion2 = new PdfPCell(TableCuentaCabecera2);
               cellDeclaracion2.setBorder(Rectangle.NO_BORDER);
               cellDeclaracion2.setPaddingLeft(32f);



                TableCuentaCabecera.addCell(cellDeclaracion2);
                ...
                PdfPTable TableSeparacion= reportUtil.crearTabla(1,90,medidaCeldas);
                ...
                PdfPTable TableBalance= reportUtil.crearTabla(4,90,medidaCeldas);
                ...
                float[] medidaCeldas2 = {7f,5f};
                medidaCeldas2 = new float[]{1.5f};
                PdfPTable TableSeparacion_space2= reportUtil.crearTabla(1,90,medidaCeldas2);
                TableSeparacion_space2.getDefaultCell().setHorizontalAlignment(Element.ALIGN_LEFT);
                cellSeparador=new PdfPCell();
                cellSeparador= reportUtil.datosTabla("  ", Element.ALIGN_LEFT);
                TableSeparacion_space2.addCell(cellSeparador);
                document.add(TableSeparacion_space2);


                //tabla detalles movimientos
                medidaCeldas = new float[]{0.55f,0.55f , 1.0f, 0.55f, 0.55f, 0.55f, 0.55f};
                PdfPTable TableMovimiento= reportUtil.crearTabla(7,90,medidaCeldas);

                //////////Info PL////////////

                List<List<String>> tabla = new ArrayList<List<String>>();
                tabla = reportsPdfServicio.consultarTablaEdoCuenta(num_cta.split(" ", 2)[0].trim(),mes,anio, usuario);
...
//                document.add(chunkSeparador);
//
                TableMovimiento= reportUtil.crearTabla(7,90,medidaCeldas);

                medidaCeldas = new float[]{1.5f};
                PdfPTable TableSeparacion_space= reportUtil.crearTabla(1,90,medidaCeldas);
...
//                        primeraDescripcion = pageConfig.getFieldLabel("lastpagebalance");
                    ...
                    if(tabla.get(i).get(3).equalsIgnoreCase("&nbsp;"))
                        ...
                        cell =reportUtil.datosTabla(tabla.get(i).get(3), Element.ALIGN_CENTER);
                    TableMovimiento.addCell(cell);
    ...
    /**
     * LISTO
     * Reporte de estado de cuenta fondo mutual
     * @return SUCCESS
     * @throws Exception
     */
    public String reportEdoCuentaFondoMutual() throws Exception {
        reporte="EDCFM";
        final String origen = "reportPdfAction.reportEdoCuentaFondoMutual";
        long time;
        Color backcolor          = new Color(192,192,192);//new Color(245,245,245);
        int   sizeLetraTitulo    = 16;//19;
        int   sizeLetraSubtitulo = 12;//14;
        ...
        try{
            ...
            writer.setPageEvent(event);
            //Agregamos un titulo al pdf
            document.addTitle("Estado de Cuenta Fondo Mutual");
            //Agregamos el autor
            document.addAuthor("VBT");
...
//                        primeraDescripcion = pageConfig.getFieldLabel("lastpagebalance");
                ...
                document.add(TableMovimiento);
                document.close();
                this.getReportsPdfServicio().guardarLog(usuario.getLogin(),"13","2","6",num_cta.trim(),usuario.getIP(),"Edo. Cuenta Fondo Mutual Fecha:"+mes+"/"+anio+ " (N° Cartera:" + 	num_cta.split(" ")[2].trim() + ")");
                inputStream  =  new ByteArrayInputStream(buffer.toByteArray());
            }else{
                throw (new Exception ("No Existe",null));
            }
    ...
    public String reportColOperation() throws Exception {
        ...
        try{
            ...
            cartera=StringUtils.newStringUtf8(Base64.decodeBase64(cartera.getBytes()));
            ...
            cabecera = reportsPdfServicio.consultarDatosCliente(cartera, usuario);
            ///
...
//        TableMovimiento.addCell(new Phrase("Balance Brought Forward",FontFactory.getFont(FontFactory.TIMES_ROMAN, 8, Font.BOLD, BaseColor.BLACK)));
    ...
    public String reportaccountBT()  {
        ...
        try{
            ...
            referencia = NullFormatter.formatBlank(StringUtils.newStringUtf8(Base64.decodeBase64(referencia.getBytes())));
            tasadecambio = StringUtils.newStringUtf8(Base64.decodeBase64(tasadecambio.getBytes()));

            descripcion = NullFormatter.formatBlank(StringUtils.newStringUtf8(Base64.decodeBase64(descripcion.getBytes())));
            tipMov = StringUtils.newStringUtf8(Base64.decodeBase64(tipMov.getBytes()));
            cartera = StringUtils.newStringUtf8(Base64.decodeBase64(cartera.getBytes()));

            beneficiario = NullFormatter.formatBlank(StringUtils.newStringUtf8(Base64.decodeBase64(beneficiario.getBytes())).equalsIgnoreCase("FALSE") ? "" : StringUtils.newStringUtf8(Base64.decodeBase64(beneficiario.getBytes())));
            observacion = NullFormatter.formatBlank(StringUtils.newStringUtf8(Base64.decodeBase64(observacion.getBytes())));
            ...
            if (
                    ("CHC".equalsIgnoreCase(tipMov)) //||
                //("CHC".equalsIgnoreCase(tipMov))
                    ) {
                // FORMATO 1
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Check Number: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Place: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("CIC".equalsIgnoreCase(tipMov)) //||
                //("CIC".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 2
                cellDetalle=reportUtil.datosTablaEncabezado("Client Id - Source: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Account N°- Source: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Originator: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("CID".equalsIgnoreCase(tipMov)) //||
                //("CID".equalsIgnoreCase(tipMov))
                    ) {
                // FORMATO 3
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Client Id - Destiny: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Account N° - Destiny: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("DP".equalsIgnoreCase(tipMov)) ||
                            ("DPE".equalsIgnoreCase(tipMov))// ||
                //("PD".equalsIgnoreCase(tipMov))
                    ) {
                // FORMATO 4
                cellDetalle=reportUtil.datosTablaEncabezado("Originator: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Issuer Bank: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("LOP".equalsIgnoreCase(tipMov)) ||
                            ("LOR".equalsIgnoreCase(tipMov))// ||
                //("PD".equalsIgnoreCase(tipMov))
                    ) {
                // FORMATO 5
                cellDetalle=reportUtil.datosTablaEncabezado("Loan Date: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Term: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Interest Rate: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("RCH".equalsIgnoreCase(tipMov)) //||
                //("PD".equalsIgnoreCase(tipMov))
                    ) {
                // FORMATO 6
                cellDetalle=reportUtil.datosTablaEncabezado("Originator: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
            ...
            } else if ("STP".equalsIgnoreCase(tipMov)) {
                // FORMATO 7
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Description: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("TEI".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 8
                cellDetalle=reportUtil.datosTablaEncabezado("Originator: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Originators Bank: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Receiver Information: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("TEO".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 9
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary Bank: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary Information: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("TIC".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 10
                cellDetalle=reportUtil.datosTablaEncabezado("Client name - Source: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("Client Id - Source: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

                cellDetalle=reportUtil.datosTablaEncabezado("TD - Source: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("TID".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 11
                cellDetalle=reportUtil.datosTablaEncabezado("Client name - destiny: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("TD - Destiny: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("MFD".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 12
                cellDetalle=reportUtil.datosTablaEncabezado("Beneficiary: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Company name - Destiny: "+( ("".equalsIgnoreCase(observacion)) ?" ":observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);

            } else if (
                    ("MFC".equalsIgnoreCase(tipMov)) //||
                //("TEI".equalsIgnoreCase(tipMov)
                    ) {
                // FORMATO 13
                cellDetalle=reportUtil.datosTablaEncabezado("Client Id - Source: "+( ("".equalsIgnoreCase(beneficiario)) ?" ":beneficiario),Element.ALIGN_LEFT);
                ...
                cellDetalle=reportUtil.datosTablaEncabezado("Company name - Source: "+( ("".equalsIgnoreCase(descripcion)) ?" ":descripcion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);
                ...
                // FORMATO 0 - GENERAL
                cellDetalle=reportUtil.datosTablaEncabezado("Observation: "+( ("".equalsIgnoreCase(observacion)) ? " " : observacion),Element.ALIGN_LEFT);
                cellDetalle.setColspan(2);
                TableOperacionDetalle.addCell(cellDetalle);
//            monto=("".equalsIgnoreCase(monto)) ? descripcion:monto;
...
//            cell =reportUtil.datosTabla("USD$");
            if (moneda.equalsIgnoreCase("USD")){
                ...
                cell =reportUtil.datosTabla(moneda);
            }
...
//        TableMovimiento.addCell(new Phrase("Balance Brought Forward",FontFactory.getFont(FontFactory.TIMES_ROMAN, 8, Font.BOLD, BaseColor.BLACK)));
            ...
            cell =reportUtil.datosTabla(("".equalsIgnoreCase(fechaValor)) ?" ":fechaValor);
            cell.setBorder(Rectangle.TOP);
            TableMovimiento.addCell(cell);

            cell =reportUtil.datosTabla(("".equalsIgnoreCase(referencia)) ?" ":referencia);
            cell.setBorder(Rectangle.TOP);
            TableMovimiento.addCell(cell);

            cell =reportUtil.datosTabla(((tasadecambio.equalsIgnoreCase("")) ? "0" :tasadecambio));
            cell.setBorder(Rectangle.TOP);
            TableMovimiento.addCell(cell);
    ...
    public String reportCertificados() throws Exception {
        ...
        try{
            ...
            if(validar.validarColocacion(colocaciones,colocacion.trim()).equalsIgnoreCase("SI") ){
                ...
                cabecera = reportsPdfServicio.consultarCeritificadoApertura(colocacion.trim(), usuario);
...
//                TableCuentaCabecera.addCell(imagenCel_logo);
                ...
                //se crea una imagen en una celda con los parametros ruta, tamano x, tamano y, porcentaje
                String archivo_temp = ServletActionContext.getServletContext().getRealPath("/");
                archivo_temp += "/resources/images/comun/bgCertificados.jpg";
                PdfContentByte canvas = writer.getDirectContentUnder ( ) ;
                Image image = Image.getInstance (archivo_temp) ;
                image.scalePercent(72.0f, 70.0f);
                image.setAbsolutePosition(0, PageSize.A4.getHeight() - image.getScaledHeight()-45);
                ...
                PdfPCell letras= reportUtil.datosTablaCertificados(cabecera.get(11).replace(",","").substring(0,cabecera.get(11).lastIndexOf(" ")).concat(",").concat(cabecera.get(11).substring(cabecera.get(11).lastIndexOf(" "))),Element.ALIGN_RIGHT);
                letras.setColspan(2);
                letras.setPaddingTop(70f);
                TableCuentaCabecera.addCell(letras);


                letras = reportUtil.datosTablaCertificados("\n\n\nTo:   "+cabecera.get(0)+"\n        "+cabecera.get(13)+"\n",Element.ALIGN_LEFT);
//                letras.setColspan(2);
                TableCuentaCabecera.addCell(letras);

                letras = reportUtil.datosTablaCertificadosNegrita("No. "+colocacion,Element.ALIGN_RIGHT);
                TableCuentaCabecera.addCell(letras);

                letras = reportUtil.datosTablaCertificados(cabecera.get(1)+" - "+num_cta,Element.ALIGN_LEFT);
                letras.setColspan(2);
                TableCuentaCabecera.addCell(letras);
...
//                for(int i=0;i<6;i++){
                    letras= reportUtil.datosTablaCertificados("  ", Element.ALIGN_LEFT);
                    letras.setColspan(2);
                    letras.setPaddingTop(20f);
                    TableCuentaCabecera.addCell(letras);
//                }

                letras = reportUtil.datosTablaCertificados("We confirm your deposit as follows:",Element.ALIGN_LEFT);
                letras.setColspan(2);
                TableCuentaCabecera.addCell(letras);
                ...
                letras = reportUtil.datosTablaCertificados("    Principal ............................................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(cabecera.get(3),Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(descripcion,Element.ALIGN_RIGHT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);

                letras = reportUtil.datosTablaCertificados("    Interest Rate ......................................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                ...
                letras = reportUtil.datosTablaCertificados("    Value Date .........................................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(" ",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(fechaValor,Element.ALIGN_RIGHT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);

                letras = reportUtil.datosTablaCertificados("    Maturity Date .....................................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(fechavencimiento,Element.ALIGN_RIGHT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);

                letras = reportUtil.datosTablaCertificados("    Interest ..............................................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(cabecera.get(3),Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(cabecera.get(8),Element.ALIGN_RIGHT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);

                letras = reportUtil.datosTablaCertificados("    Total Due at Maturity..........................................................",Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(cabecera.get(3),Element.ALIGN_LEFT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados(monto,Element.ALIGN_RIGHT);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n\n\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);

                letras = reportUtil.datosTablaCertificados("Special Conditions",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("Should you require any information regarding your account(s)please feel free to contact our Customer Service Desk in the Cayman Islands at 345-040-6917. fax 345-040-8017 from 8:00 a.m. to 4:30 p.m. (EST) from monday to friday or by e-mail address: clientservices@vbtbank.com",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("Origin of funds:",Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
                ...
                List<List<String>> tabla = new ArrayList<List<String>>();
                tabla = reportsPdfServicio.consultarmovimientosCertificado(colocacion.trim(),estatus, usuario);
                ...
                for(int f=0;f<tabla.size();f++){
                    ...
                    letras = reportUtil.datosTablaCertificados(tabla.get(f).get(0),Element.ALIGN_LEFT);
                    TableCertificados2.addCell(letras);
                    letras = reportUtil.datosTablaCertificados(tabla.get(f).get(5).equalsIgnoreCase("&nbsp;")?" ":tabla.get(f).get(5),Element.ALIGN_LEFT);
                    TableCertificados2.addCell(letras);
                    letras = reportUtil.datosTablaCertificados(tabla.get(f).get(4),Element.ALIGN_RIGHT);
                    TableCertificados2.addCell(letras);
                    letras = reportUtil.datosTablaCertificados(tabla.get(f).get(1),Element.ALIGN_RIGHT);
                    TableCertificados2.addCell(letras);
                }
...
//                }
                ...
                letras = reportUtil.datosTablaCertificados("Your deposit No.  "+colocacion+" is being repaid as follows:",Element.ALIGN_LEFT);
                letras.setColspan(2);
                TableCuentaCabecera.addCell(letras);
                //letras = reportUtil.datosTablaCertificados("is being repaid as follows:",Element.ALIGN_LEFT);
                //letras.setColspan(2);
                //TableCuentaCabecera.addCell(letras);
...
//                }
                ...
                letras = reportUtil.datosTablaCertificados("We credit: "+cabecera.get(12),Element.ALIGN_LEFT);
                letras.setColspan(3);
                TableCertificados.addCell(letras);
                letras = reportUtil.datosTablaCertificados("\n\n",Element.ALIGN_LEFT);
                letras.setColspan(3);
    ...
    public String reportEdoCuentaTDC() throws Exception {
        ...
        try{
            ...
            if(validar.validarTDC(tarjetas,num_cta.trim()).equalsIgnoreCase("SI")){
                cabecera = reportsPdfServicio.consultarCabeceraEdoCuentaTDC(num_cta.trim(),codproserv,mes, usuario);
                CurrencyFormatter formatoMonto = new CurrencyFormatter();
                ...
                List<List<String>> tabla = new ArrayList<List<String>>();
                tabla = reportsPdfServicio.consultarTablaEdoCuentaTDC(num_cta.trim(),codproserv,mes, usuario);
...
//              celldatos = new PdfPCell(reportUtil.formatoValor(8,"TRANSACTIONS SUB-TOTAL.... "+String.valueOf(decf.format(saldo))));
    ...
    class HeaderFooter extends PdfPageEventHelper {
        ...
        public void onCloseDocument(PdfWriter writer, Document document) {
            ...
            frase =new Phrase();
            frase.setFont(FontFactory.getFont(FontFactory.HELVETICA_BOLD, 7, com.itextpdf.text.Font.BOLD, BaseColor.BLACK));
        ...
        public void piePagina_EC(PdfWriter writer, Document document) throws DocumentException {
            try {
                ...
                //mbernot cambio auditor

                String Mensline1="Please review and report any discrepancies to our auditors Grant Thornton, P.O. Box 1044, Grand Cayman, Cayman Islands, KY1-1102 ";
                //String Mensline1="Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 ";
                //String Mensline1="Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 ";
                                //"PLEASE REVIEW AND REPORT ANY DISCREPANCIES TO OUR AUDITORS DELOITTE & TOUCHE AT ONE CAPITAL PLAZA, P.O. BOX 1787, GRAND CAYMAN";
                //String Mensline2="(to the attention of Thomas Marks [phone +1 345 815 26 54] / Ranga Bernanduge [phone +1 345 914 47 43]), or alternatively,";

                String Mensline2="(to the attention of Cormac Golden [phone +1 345 949 8588]), or alternatively,";
                //String Mensline2="(to the attention of Thomas Marks [phone +1 345 815 26 54] / Joseph Muhia [phone +1 345 916 68 58]), or alternatively,";

                //String Mensline2="(to the attention of Curran Patel [phone +1 345 914 44 22] / Angelica Lejarde [phone +1 345 914 43 62]), or alternately, ";
                //"KY1-1109, CAYMAN ISLANDS. TO THE ATTENTION OF JAMIE FEARN EMAIL JFEARN@DELOITTE.COM TEL NUMBER: (345) 814-3436 AND ";
                //String Mensline3="you could communicate any exceptions by email to thomasmarks@kpmg.ky / josephmuhia1@kpmg.ky.";

                String Mensline3="you could communicate any exceptions by email to Cormac.Golden@ky.gt.com  / Desigan.Thaver@ky.gt.com.";
                //String Mensline3="you could communicate any exceptions by email to thomasmarks@kpmg.ky / josephmuhia1@kpmg.ky.";

                //String Mensline3="you could communicate any exceptions by email to curranpatel1@kpmg.ky /alejarde@kpmg.ky.";//"FAX NUMBER: (345) 949-8238.";
...
//                String PiedePagina2 = "P.O.Box 454, 4th Floor, Flagship Building, 70 Harbour Drive, George Town, Grand Cayman KY1-1106, CAYMAN ISLANDS";
                String PiedePagina2 = "2nd Floor, The Harbour Centre. 42 North Church Street. George Town. PO Box 454. Grand Cayman KY1- 1106 Cayman Islands.";
                String PiedePagina3 = "Phone (345)949-6917. Fax (345)949-8017. e-mail: clientservices@vbtbank.com Website: www.vbtbank.com";
                //viejo:PO Box 454, 2nd Floor, The Harbour Centre, 42 North Church Street, George Town, Grand Cayman KY1-1106, Cayman Islands
                //actual:2nd Floor, The Harbour Centre. 42 North Church Street. George Town. PO Box 454. Grand Cayman KY1- 1106 Cayman Islands. Phone (345) 949-6917 - Fax (345) 949-8017 - Email: clientservices@vbtbank.com - Website: www.vbtbank.com
                PdfPTable TablePie_pag = new PdfPTable(1);
                PdfPTable Pg = new PdfPTable(2);

                TablePie_pag.setTotalWidth(540);
                TablePie_pag.setHorizontalAlignment(Element.ALIGN_RIGHT);
                TablePie_pag.getDefaultCell().setHorizontalAlignment(Element.ALIGN_RIGHT);
                ...
                // SE COMENTA EL PIE DE PAGINA AUDITORES SOLICITADO POR MARGOTT 2020
...
//                }
                ...
                celldatos.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
                celldatos.setBorder(Rectangle.NO_BORDER);
                celldatos.setPaddingLeft(6f);
                //celldatos.setColspan(2);
                Pg.addCell(celldatos);

                TablePie_pag.addCell(Pg);
                TablePie_pag.writeSelectedRows(0, -1, 50, 90, writer.getDirectContent());
        ...
        public PdfPTable piePagina_TDC2(PdfWriter writer, Document document) throws DocumentException {
            ...
            String msgPiedePagina2 = "If you have questions about your statement, you can either call your Financial Advisor/Account Executive or e-mail clientservices@vbtbank.com to get in touch directly with VBT Bank & Trust, Ltd.";
            //mbernott cambio auditor
            //String msgPiedePagina3 = "Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 (to the attention of Thomas Marks [phone +1 345 815 26 54] / Ranga Bernanduge [phone +1 345 914 47 43]), or alternatively, you could communicate any exceptions by email to thomasmarks@kpmg.ky / rbernanduge@kpmg.ky.";

            //2020
            String msgPiedePagina3 = "Please review and report any discrepancies to our auditors Grant Thornton, P.O. Box 1044, Grand Cayman, Cayman Islands, KY1-1102 (to the attention of Cormac Golden [phone +1 345 949 8588]), or alternatively, you could communicate any exceptions by email to Cormac.Golden@ky.gt.com  / Desigan.Thaver@ky.gt.com.";
            ...
            //String msgPiedePagina3 = "Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 (to the attention of Curran Patel [phone +1 345 914 44 22] / Angelica Lejarde [phone +1 345 914 43 62]), or alternately, you could communicate any exceptions by email to curranpatel1@kpmg.ky /alejarde@kpmg.ky.";//"PLEASE REVIEW AND REPORT ANY DISCREPANCIES TO OUR AUDITORS DELOITTE & TOUCHE AT ONE CAPITAL PLAZA, P.O. BOX 1787 GRAND CAYMAN KY1-1109, CAYMAN ISLANDS. TO THE ATTENTION OF JAMIE FEARN EMAIL JFEARN@DELOITTE.COM TEL NUMBER: (345) 814-3436 AND FAX NUMBER: (345) 949-8238.";
            ...
            // SE COMENTA PIE DE PAGINA DE LOS AUDITORES MARGOTT 2020
        ...
        public PdfPTable piePagina_TDC(PdfWriter writer, Document document) throws DocumentException {
            ...
            // ESTA COMENTADO PIE DE PAGINA AUDITORIA POR ORDEN DE MARGOTT 2020
        ...
        public void piePagina_certificados(PdfWriter writer, Document document) throws DocumentException {
            try {
                ...
                String PiedePagina1 = "In accordance with the applicable laws of the Cayman Islands and pursuant to a special resolution of the sole shareholder of Venecredit Bank & Trust, Ltd. (the \"Bank\") passed on 24 February 2012, "+
                 ...
                 //mbernott cambio auditor
                String PiedePagina2 = "Please review and report any discrepancies to our auditors Grant Thornton, P.O. Box 1044, Grand Cayman, Cayman Islands, KY1-1102 (to the attention of Cormac Golden [phone +1 345 949 8588]), or alternatively, you could communicate any exceptions by email to Cormac.Golden@ky.gt.com  / Desigan.Thaver@ky.gt.com.";
                //String PiedePagina2 = "Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 (to the attention of Thomas Marks [phone +1 345 815 26 54] / Ranga Bernanduge [phone +1 345 914 47 43]), or alternatively, you could communicate any exceptions by email to thomasmarks@kpmg.ky / rbernanduge@kpmg.ky.";
                //String PiedePagina2 = "Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 (to the attention of Thomas Marks [phone +1 345 815 26 54] / Joseph Muhia [phone +1 345 916 68 58]), or alternatively, you could communicate any exceptions by email to thomasmarks@kpmg.ky / josephmuhia1@kpmg.ky.";
                //String PiedePagina2 = "Please review and report any discrepancies to our auditors KPMG, P.O. Box 493 GT, Grand Cayman, Cayman Islands, KY1-1106 (to the attention of Curran Patel [phone +1 345 914 44 22] / Angelica Lejarde [phone +1 345 914 43 62]), or alternately, you could communicate any exceptions by email to curranpatel1@kpmg.ky /alejarde@kpmg.ky.";
                /*"PLEASE REVIEW AND REPORT ANY DISCREPANCIES TO OUR AUDITORS DELOITTE & TOUCHE AT ONE CAPITAL PLAZA, P.O. BOX 1787, GRAND CAYMAN \n" +
                        "KY1-1109, CAYMAN ISLANDS. TO THE ATTENTION OF JAMIE FEARN EMAIL JFEARN@DELOITTE.COM TEL NUMBER: (345) 814-3436 AND \n" +
