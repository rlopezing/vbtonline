...
public class AccountsAction extends ActionSupport implements ServletContextAware, SessionAware,Serializable {
    ...
    private String jsonComentarioTrans;
    private String mes;
    private String anio;
    ...
    public String cargar() throws Exception {
        ...
        try{
            ...
            account=getAccountsServicio().cargar("", data.getAccountss().get(0), usuario);
...
//            data = gson.fromJson(this.jsonAccounts, DataJson.class);
            ...
            cuentasDescrip= getAccountsServicio().cargarCuentasEdoCuenta(carteras, idioma, usuario);
            cuentas=cuentasDescrip.getContenido();
            session.put("cuentasUsuario",cuentas);
            fechaCierreAS = cuentasDescrip.getFecha();
            usuario = (VBTUsersOd) session.get("user");
            String x = usuario.toString()  ;
...
//
    ...
    public String consultarAccountStatement() throws Exception {
        ...
        try{
            ...
            if(validar.validarCuenta(carteras, cuentas, data).equalsIgnoreCase("SI")){
                ...
                detalle= getAccountsServicio().consultarDetalleCuentasEdoCuenta(parametros, usuario);
                idioma = (String) session.get("idioma");
                ...
                TableOd tabla = getAccountsServicio().consultarCuentasEdoCuenta(parametros, saldoAnterior, idioma, usuario);
                ...
                List<ContentsTableColumnsOd> listac = tabla.getContenidoTabla_culumnas();
                List<ContentTableInfoOd> listai = tabla.getContenidoTabla_info();


                for (int a=0;a<listac.size();a++){
                    ContentsTableColumnsOd item =  listac.get(a);
                }


                for (int a=0;a<listai.size();a++){
                    ContentTableInfoOd item =  listai.get(a);
                    List<DataColumnOd> listad = item.getData_culumn();
                }
    ...
    public String guardarComentarioTrans() throws Exception {
        final String origen = "accountAccion.guardarComentarioTrans";
        long time;
        List<String> parametros;
        idioma = (String) session.get("idioma");
        VBTUsersOd usuario = (VBTUsersOd) session.get("user");
        try{
            ...
            time = System.currentTimeMillis ();
            Gson gson=new Gson();
            data = gson.fromJson(this.jsonComentarioTrans, DataJson.class);
            parametros = new ArrayList<String>();
            parametros.add(data.getParametros().get(0));
            parametros.add(data.getParametros().get(1));
            respuesta = getAccountsServicio().guardarComentarioTrans(parametros,usuario);
            time = System.currentTimeMillis () - time;
            this.GuardarLog(usuario.getLogin(),"3","1","4", data.getParametros().get(0).trim(),usuario.getIP(),"Movimiento:" +  data.getParametros().get(0) + ")");
            logger.info(ResourceBundle.getBundle("mensajeslogs").getString("mensaje.exitos.act")+AccountsAction.class+" | "+origen+" | "+time);
            ...
            if(idioma.equalsIgnoreCase("_ve_es"))
                setMensaje("Ha ocurrido un error, por favor intente mas tarde");
            else
                setMensaje("An error has occurred, please try later");
            respuesta = "NO OK";
        }
    ...
    public String consultarCuentaSaldoTrans() throws Exception {
        ...
        try{
            ...
            if(validar.validarCuenta(carteras, cuentas, data).equalsIgnoreCase("SI"))
            {
                ...
                idioma = (String) session.get("idioma");
                TableOd tabla = getAccountsServicio().consultarCuentasSaldoTransf(parametros, idioma, usuario);
    ...
    public String consultarCuentasBloqueos() throws Exception {
        ...
        List<String> parametros;
        VBTUsersOd usuario = (VBTUsersOd) session.get("user");
        fechaCierre = getAccountsServicio().consultarFechaCierre((String) session.get("carteras"), usuario);
        validacionUtil validar = new validacionUtil();
        try{
            ...
            if(validar.validarCuenta(carteras, cuentas, data).equalsIgnoreCase("SI"))
            {
                ...
                idioma = (String)  session.get("idioma");
                TableOd tabla = getAccountsServicio().consultarCuentasBloqueos(parametros, idioma, usuario);
    ...
    public String consultarBalancesAndTransaction() throws Exception {
        ...
        try{
            ...
            if(validar.validarCuenta(carteras, cuentas, data).equalsIgnoreCase("SI"))
            {
                ...
                detalle = new DetalleCuentaEdoCtaOd();
                detalle= getAccountsServicio().consultarDetalleCuentasSaldoTrans(parametros, usuario);
                ...
                tipoTransacciones = getAccountsServicio().cargarTiposMovimientosBT(parametros, usuario);
                codigo = "1";
    ...
    public String consulta() throws Exception {
        ...
        try{
            ...
            TableOd tableOd=getAccountsServicio().Consulta("",account, usuario);
    ...
    public String getJsonComentarioTrans() {
        return jsonComentarioTrans;
    }

    public void setJsonComentarioTrans(String jsonComentarioTrans) {
        this.jsonComentarioTrans = jsonComentarioTrans;
    }
