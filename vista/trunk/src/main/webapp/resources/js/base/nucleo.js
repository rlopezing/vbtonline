    ...
    $.each(lista,function(s,item){
        ...
        if(item.seleccionado!=null){
            if(item.seleccionado=="SI")
              option.attr('selected', 'selCU_searchected');
        }
...
function cargar_selectExtra(select,lista){
    var select=$("#"+select);
    select.empty();
    var option;
    option = $(document.createElement('option')).appendTo(select);
...
function cargar_selectEmail(select,lista){
    var select=$("#"+select);
    select.empty();
    var option;
    option = $(document.createElement('option')).appendTo(select);
...
function cargar_selectPersonal(selecti,lista,campo,valor){
    ...
    $.each(lista,function(s,item){
        ...
        option.attr("value",item.value);
        option.attr("extra",item.extra);
        option.attr("extra1", item.extra1);
        option.attr("extra2", item.extra2);
        option.attr("extra3", item.extra3);
        option.attr("extra4", item.extra4);
        option.attr("extra5", item.extra5);
        option.attr("extra6", item.extra6);
        option.attr("extra7", item.extra5);
        option.attr("extra8", item.extra6);
        option.attr("valor", item.valor);
        option.html(item.label);
    });
}

function cargar_selectPersonalpaisesMod(selecti,lista,campo,valor,opcion){
    var select=$("#"+selecti);
    select.empty();
    var option;
    option = $(document.createElement('option')).appendTo(select);
    ...
    if(opcion=="1"){
        option = $(document.createElement('option')).appendTo(select);
        option.attr("value","VE");
        option.html("Venezuela");
    }
    option = $(document.createElement('option')).appendTo(select);
    option.attr("value","US");
    option.html("United States");
    option = $(document.createElement('option')).appendTo(select);
    option.attr("value","-1");
...
function cargar_selectAtributosPersonal(selecti,lista,campo,valor){
    var select=$("#"+selecti);
    select.empty();
    var option;
    option = $(document.createElement('option')).appendTo(select);
    ...
    $.each(lista,function(s,item){
        option = $(document.createElement('option')).appendTo(select);
        option.attr("value",item.value);
        option.attr("soporte",item.soporte);
        option.attr("descripcion",item.requerido);
        option.html(item.label);
    });

}

function cargar_selectCuenta(selecti,lista,campo,valor,defecto, mostrarSeleccione){
    var select=$("#"+selecti);
    ...
    mostrarSeleccione = mostrarSeleccione || '1';
    if(mostrarSeleccione == '1'){
        option = $(document.createElement('option')).appendTo(select);
        option.attr("value",valor);
        option.attr("id",selecti+"Select");
        option.html(campo);
...
function cargar_timeDeposits(selecti,lista,campo,valor,defecto, mostrarSeleccione){
    var select=$("#"+selecti);
    ...
    $.each(lista,function(s,item){
        ...
        option.attr("value",item.value);
        option.attr("extra", item.extra);
        option.attr("plazoTD", item.extra1);
        option.attr("plazoFinTD", item.extra2);
        option.html(item.label);
    });
    select.val(defecto);
}
...
function cargar_selectPersonal2(select,lista){
    ...
    $.each(lista,function(s,item){
        option = $(document.createElement('option')).appendTo(select);
        option.attr("value",Trim(item.value));
        option.html(Trim(item.label));
    });
...
function cargar_selectPersonalId(selecti,lista,campo,valor){
    var select=$("#"+selecti);
    select.empty();
    var option;
    option = $(document.createElement('option')).appendTo(select);
    option.attr("value",valor);
    option.attr("id", selecti+valor);
    option.html(campo);
    $.each(lista,function(s,item){
        option = $(document.createElement('option')).appendTo(select);
        option.attr("value",item.value);
        option.attr("id", selecti+item.value);
        option.html(item.label);
    });
...
function createCurrency(value){
    if(value=="USD"){
        return `<div class="currency">
                    <img class="currency__icon"
                         src="../vbtonline/resources/img/ic_wiretransfer_instruction_usd.png"
                         alt=""/>
                    <div class="currency__titles">
                        <span class="currency__title">USD</span><span class="currency__description">US Dollars</span>
                    </div>
                </div>`;
    } else if(value=="EUR"){
        return `<div class="currency">
                    <img class="currency__icon"
                         src="../vbtonline/resources/img/ic_wiretransfer_instruction_eur.png"
                         alt=""/>
                    <div class="currency__titles">
                        <span class="currency__title">EUR</span><span class="currency__description">Euros</span>
                    </div>
                </div>`;
    } else{
        return value;
...
function onlyDigits_v2(e, value, allowDecimal, allowNegative, allowThousand, decSep, thousandSep, decLength,objeto){
    ...
    if(key == 46){
        ...
        if (objeto=="EM"){
            ...
            if ($("#otroMontoPagoTDC").val().indexOf('.')!=-1) {
                return false;
            }
        }
    }
    ...
    if (objeto=="EM"){
        ...
        if ($("#otroMontoPagoTDC").val().indexOf('.')!=-1) {
            arr=$("#otroMontoPagoTDC").val().split(".");
            validar=true;
        }
    }
//    if ((validar)&&(key!=51))
...
function crearTablaV2(id_tabla,contenidoTabla_culumnas,contenidoTabla_info, row_style, isLastItem){

    var tabla=$("#"+id_tabla);
    tabla.empty();
    console.log(contenidoTabla_culumnas)
    console.log(contenidoTabla_info)

    var thead=$(document.createElement('thead')).appendTo(tabla);
    var tr= $(document.createElement('tr')).appendTo(thead);
    tr.addClass("table__row")
    tr.addClass(row_style)
    ...
    $.each(contenidoTabla_culumnas,function(c,item){
        var columna=contenidoTabla_culumnas[c];
        th= $(document.createElement('th')).appendTo(tr);
        th.addClass("table__item");
        th.addClass("table__item--head")
        if(isLastItem){
            var length=contenidoTabla_culumnas.length-1;
            if(length==c){
                th.addClass("table__item--last")
            }
        }
        th.attr("id","tag_"+columna.span_html.toString().replace(" ","_").substring(0,columna.span_html.toString().lastIndexOf(" ")>0?columna.span_html.toString().lastIndexOf(" "):columna.span_html.toString().length)+"_consolidado_det");
        th.html(columna.span_html);
    ...
    $.each(contenidoTabla_info,function(dc,item){
        ...
        tr.addClass(row_style)
        $.each(data.data_culumn,function(d,item){
            var column=contenidoTabla_culumnas[d];
            var data_info=data.data_culumn[d];
            td= $(document.createElement('td')).appendTo(tr);
            td.addClass("table__item");
            td.attr("data-label", column.span_html);
            if(isLastItem){
                var length=data.data_culumn.length-1;
                if(length==d){
                    td.addClass("table__item--last")
                }
            }
            // td.html(data_info.data_columna);
            td.html(createCurrency(data_info.data_columna));
        });
...
// const paginaFullNumbers = document.querySelectorAll(".paging_full_numbers");

/*$(".paginate_button").each((index, element) => {
    console.log(index);
    console.log(element);
...
function crearTablaV3(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info,row_style,isLastItem){
    var div=$("#"+div_contenedor);
    div.empty();

    var tabla = $(document.createElement('table')).appendTo(div);
    tabla.attr("id",id_tabla);
    tabla.addClass("table__content")
...
function crearTablaV4(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info,row_style,isLastItem){
    var div=$("#"+div_contenedor);
    div.empty();
    ...
    $.each(contenidoTabla_culumnas,function(c,item){
        ...
        th.attr("id","tag_"+columna.span_html.toString().replace(" ","_").substring(0,columna.span_html.toString().lastIndexOf(" ")>0?columna.span_html.toString().lastIndexOf(" "):columna.span_html.toString().length)+"_consolidado_det");
        th.html(columna.span_html.toString().split("#")[1]);
        //th.html("<span id='tag_"+columna.span_html.toString().substring(0,columna.span_html.toString().indexOf(" ")!=-1?columna.span_html.toString().indexOf(" "):columna.span_html.toString().length-1)+"_consolidado_det'>"+columna.span_html+"</span>");
    });
...
function crearTablaAvisos(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info){
    var div=$("#"+div_contenedor);
    div.empty();
    var tabla = $(document.createElement('table')).appendTo(div);
    tabla.attr("cellpadding","0");
    ...
    $.each(contenidoTabla_info,function(dc,item){
        ...
        $.each(data.data_culumn,function(d,item){
            var data_info=data.data_culumn[d];
            if(d<2)
                data_info.data_columna.replace("=","'");
            td= $(document.createElement('td')).appendTo(tr);
            td.attr("class","");
//            td.attr("class","center");
            td.html(data_info.data_columna);
...
function crearTablaSinDiv(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info){
...
function crearTablaTmpV2(id_tabla,contenidoTabla_culumnas,contenidoTabla_info,row_style){
    var tabla = $("#"+id_tabla);
    tabla.empty();
...
function crearTabla_i18n(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info){
    var div=$("#"+div_contenedor);
    div.empty();
    var tabla = $(document.createElement('table')).appendTo(div);
    tabla.attr("cellpadding","0");
    ...
    $.each(contenidoTabla_culumnas,function(c,item){
        ...
        th.attr("class","center");
        //th.attr("id","tag_"+columna.span_html.toString().replace(" ","_")+"_detalle");
        th.attr("id",columna.span_html.toString().split("#")[0]);
        th.html(columna.span_html.toString().split("#")[1]);
    });
...
function crearTabla_i18nV2(id_tabla,contenidoTabla_culumnas,contenidoTabla_info,row_style){
    var tabla = $("#"+id_tabla);
    tabla.empty();
...
function crearTabla_paginacion(div_contenedor,id_tabla,contenidoTabla_culumnas,contenidoTabla_info,reg_inicio,tam_pagina,p_sclass){
    ...
    div_contenedor.attr("class", "display");
...
function crearTabla_paginacionV2(id_tabla,contenidoTabla_culumnas,contenidoTabla_info,reg_inicio,tam_pagina,row_style){
    var tabla = $("#"+id_tabla);
    tabla.empty();
...
idleTime = 0;
function idleTimeOut(minutesTimeOut){
    //Increment the idle time counter every minute.
    var idleInterval = setInterval("timerIncrement(minutesTimeOut)", 60000); // 1 minute

    //Zero the idle timer on mouse movement.
    $(document).click(function (e) {
        if(e.target.className==='dataTables_wrapper'){
...
function timerIncrement(minutesTimeOut) {
    idleTime = idleTime + 1;
    if (idleTime == minutesTimeOut){ // se hace un llamado a la función que emite el mensaje cuando el usuario se mantiene inactivo 4 minutos.
        loadAlertTimeout();
    }
...
function validarn(e) { // 1
    ...
    if ((tecla>=48)&&(tecla<=59)) return true; // 3
